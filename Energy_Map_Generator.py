...
def build_model(nb_channels):
	...
	u_path = Activation(activation='relu')(u_path)
...
def train_model(in_folders, mask_folders, block_sizes, channels=1, num_epochs=200, batch_size=10, validation_split=0.1, lr=1, reduce_lr_factor=0.5, loss_delta=0.001, patience=5):
	...
	# model.summary()
	prefix = '/scratch/unet_lstm/mae'

	# videos_list, targets_list, video_names = read_videos(in_folders, mask_folders)
	model.summary()
	# model.load_weights('batch_model.h5')
	...
	if not os.path.isfile(os.path.join(prefix, 'Training_statistics.csv')):
		...
		train_stats.write('Epoch,Loss,Metric,%s(Loss),%s(Metric),%s(Loss),%s(Metric)\n'%('480p','480p','480p(scaled)','480p(scaled)'))
		train_stats.close()

		val_stats = open(os.path.join(prefix, 'Validation_statistics.csv'), 'w')
		val_stats.write('Epoch,Loss,Metric,%s(Loss),%s(Metric),%s(Loss),%s(Metric)\n'%('480p','480p','480p(scaled)','480p(scaled)'))
		val_stats.close()
...
def generate_saliency_map(in_folder, out_folder, block_size=16):
	...
	model.summary()
	model.load_weights(os.path.join('/scratch/unet_lstm/mae', 'model_epoch_2.h5'))
	...
	for count in range(frame_count):
		...
		image = np.array([np.expand_dims(frame.astype(np.float64) / 255, axis=2)])
...
def main(): 
	in_folders = ['/scratch/Dataset/DAVIS-2017-trainval-480p/full/images',
				  '/scratch/Dataset/DAVIS-2017-trainval-480p/scaled/images']
	mask_folders = ['/scratch/Dataset/DAVIS-2017-trainval-480p/full/masks',
					'/scratch/Dataset/DAVIS-2017-trainval-480p/scaled/masks']
	block_sizes = [16, 16]
	train_model(in_folders, mask_folders, block_sizes)

if __name__=='__main__':
	main()
	# generate_saliency_map('/scratch/Dataset/DAVIS-2017-trainval-480p/full/images/tractor-sand', '/scratch/network_output/unet_lstm')

	# files = os.listdir('/scratch/network_output/unet_lstm')
	# for file in files:
	# 	if file.endswith('.npy'):
	# 		path = os.path.join('/scratch/network_output/unet_lstm', file)
	...
	# 		cv2.imwrite(os.path.join('/scratch/network_output/unet_lstm/images', file.split('.')[0] + '.png'), np.uint8(saliency_map * 255))
